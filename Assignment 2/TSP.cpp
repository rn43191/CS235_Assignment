...
//find vertext in open and update the total cost and path cost
...
int h_x(int v, int *adj[], vector<int> leftItems)
{
    ...
    Graph graph(leftItems.size());
    for (int it = 0; it < leftItems.size(); ++it)
    {   
        for (int jt = it + 1; jt < leftItems.size(); ++jt)
        {
            {
                graph.addEdge(it, jt, adj[it][jt]);
            }
        }   
    }
...
int a_Star(int **adj, int V)
{
    ...
    while (!leftItems.empty())
    {
        ...
        for (int i = 0; i < leftItems.size(); ++i)
        {
            if (leftItems[i] == city)
            {
                leftItems.erase(leftItems.begin() + i);
                break;
            }
        }
